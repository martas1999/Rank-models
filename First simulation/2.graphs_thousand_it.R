...
# percentage of conditions that were excluded a priori due to their unfeasibility (192 original conditions)
159/192
1-0.828125
...
summary(lm(sim_data_error ~ n_games + n_comp + n_entrants + sigma, data = sim_error_lm)) #sigma is the only non-significant predictors
...
sim_failures <-grid_results_def_final %>% 
  ...
  group_by(n_games, n_comp, n_entrants, sigma) %>% 
  summarise(
    count = n())
View(sim_failures)
...
summary(lm(beta1_error ~ n_games + n_comp + n_entrants + sigma, data = error_beta1_lm)) #sigma is the only non-significant predictors
...
beta1_failures <- grid_results_def_final %>% 
  ...
  select(n_games, n_comp, n_entrants, sigma, beta1_mse) %>%
  pivot_longer(
    cols = ends_with("mse"),
    names_to = "model",
    values_to = "mse",
  ...
  group_by(n_games, n_comp, n_entrants, sigma, model) %>% 
  summarise(
    count = n())
View(beta1_failures)
...
summary(lm(beta2_error ~ n_games + n_comp + n_entrants + sigma, data = error_beta2_lm)) #sigma is the only non-significant predictors

grid_results_def_final %>% 
  filter(sim_data_error == FALSE & is.na(beta2_mse) ) %>% 
  select(n_games, n_comp, n_entrants, sigma, beta2_mse) %>%
  pivot_longer(
    cols = ends_with("mse"),
    names_to = "model",
    values_to = "mse",
...
tidy_na_models <-grid_results_def_final %>% 
  ...
  mutate(n_games = as.factor(n_games),
         ...
         n_entrants = as.factor(n_entrants),
...
tidy_row <- grid_results_def_final %>% 
  ...
  mutate( beta0_mse = as.numeric(beta0_mse),
         ...
         sparsity = p_pair_played(n_comp, n_entrants, n_games)) %>%
  mutate( n_games = as.factor(n_games),
         n_comp = as.factor(n_comp),
         n_entrants = as.factor(n_entrants),
         sigma = as.factor(sigma),
...
tidy_delta<- tidy_row %>%
  ...
  select(n_games, n_comp, n_entrants, sigma, iter,beta0_delta, beta1_delta, beta2_delta,sparsity) %>% 
  pivot_longer(
...
Figure1 <- tidy %>% 
  ...
  ggplot(aes(x=n_entrants, y=mse, color = model)) + 
  geom_boxplot()+
...
# The approximation of the Beta models to the ROL model increases for higher
# number of entrants in high-competitive and average-competitive scenarios,
# while in low-competitive scenario the opposite pattern is observed.

# testing other levels of n_games and n_comp to see if the pattern is consistent
# (go to Figure 2 if you want to directly reproduce the second figure in the manuscript)

# n_games = 500, n_comp = 50
tidy %>% 
  filter (n_games == 500 & n_comp == 50) %>% 
  ggplot(aes(x=n_entrants, y=mse, color = model)) + 
  geom_boxplot()+
  labs(title = "MSE by competitiveness levels and number of entrants, with 50 competitors and 1250 games",
       x = "Number of entrants",
...
# n_games = 1250, n_comp = 15
tidy %>% 
  filter (n_games == 1250 & n_comp == 15) %>% 
  ggplot(aes(x=n_entrants, y=mse, color = model)) + 
  geom_boxplot()+
  labs(title = "MSE by competitiveness levels and number of entrants, with 50 competitors and 1250 games",
       x = "Number of entrants",
...
# n_games = 1250, n_comp = 250
tidy %>% 
  filter (n_games == 1250 & n_comp == 250) %>% 
  ggplot(aes(x=n_entrants, y=mse, color = model)) + 
  geom_boxplot()+
  labs(title = "MSE by competitiveness levels and number of entrants, with 50 competitors and 1250 games",
       x = "Number of entrants",
...
# n_games = 1250, n_comp = 500
tidy %>% 
  filter (n_games == 1250 & n_comp == 500) %>% 
  ggplot(aes(x=n_entrants, y=mse, color = model)) + 
  geom_boxplot()+
  labs(title = "MSE by competitiveness levels and number of entrants, with 50 competitors and 1250 games",
       x = "Number of entrants",
...
# the pattern observed in Figure 1 is confirmed also for different number of games 
# and different number of competitors.

# looking at the differences in the MSE between the Beta models' MSE and the ROL's MSE for the same scenarios of Figure1
tidy_delta %>% 
  filter (n_games == 1250 & n_comp == 50) %>% 
  ggplot(aes(x=n_entrants, y=delta, color = model)) + 
  geom_boxplot()+
...
# While ROL always improves for increasing number of games, Beta models only do that 
# in high- competitive and average-competitive scenarios.

# testing other levels of n_comp and n_entrants to see if the pattern is consistent
# (go to Figure 3 if you want to directly reproduce the third figure in the manuscript)

# n_comp == 50 and n_entrants == 2
tidy %>% 
...
# the pattern observed in Figure 2 is consistent across different levels of n_comp and n_entrants

# # looking at the differences in the MSE between the Beta models' MSE and the ROL's MSE for the same scenarios of Figure2
tidy_delta %>% 
  filter(n_comp == 50 & n_entrants == 20) %>%
...
Figure3 <- tidy %>%
  group_by(n_games, n_comp, n_entrants, sigma, model,sparsity) %>% 
  summarise(
    mean_mse = mean(mse)
  ) %>%
  rename(n_entrants = n_entrants,
         tot_comp = n_comp) %>% 
  filter(sigma == "Average-competitive")%>% 
...
# For each scenario derived from a di↵erent combination of entrants and total competitors, 
# Beta models better approximate ROL for lower densities.

# testing if the same pattern is observed also in different competitiveness levels 

# low-competitve scenarios
tidy %>%
  group_by(n_games, n_comp, n_entrants, sigma, model,sparsity) %>% 
  summarise(
    mean_mse = mean(mse)
...
# Beta-models best approximated ROL in the most sparse context

# high-competitve scenarios
tidy %>%
  group_by(n_games, n_comp, n_entrants, sigma, model,sparsity) %>% 
  summarise(
    mean_mse = mean(mse)
...
# Beta-models always well approximated ROL across different densities levels present in the data